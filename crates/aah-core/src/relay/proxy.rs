...
use super::state::RelayOwnerKind;

const DEFAULT_CODEX_CLIENT_VERSION: &str = "0.99.0";
const DEFAULT_CODEX_INSTRUCTIONS: &str = "You are a helpful coding assistant.";
pub const RELAY_HEALTH_PATH: &str = "/_aah/relay/health";
pub const RELAY_STOP_PATH: &str = "/_aah/relay/stop";
pub const RELAY_ADMIN_TOKEN_HEADER: &str = "x-aah-relay-token";
...
pub struct RelayProxyState {
    ...
    admin_control: Option<RelayAdminControl>,
}
...
impl RelayProxyState {
    ...
    pub fn with_admin_control(mut self, admin_control: RelayAdminControl) -> Self {
        self.admin_control = Some(admin_control);
        self
    }
}

#[derive(Clone)]
pub struct RelayAdminControl {
...
impl RelayAdminControl {
    ...
    fn health_response(&self) -> RelayHealthResponse {
        RelayHealthResponse {
            owner_kind: self.owner_kind,
            bind_host: self.bind_host.clone(),
            port: self.port,
            codex_base_url: self.codex_base_url.clone(),
        }
    }

    fn stop(&self) -> Result<(), StatusCode> {
        let mut shutdown = self
            .shutdown
            .lock()
            .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
        let Some(sender) = shutdown.take() else {
            return Err(StatusCode::CONFLICT);
        };
        sender.send(()).map_err(|_| StatusCode::CONFLICT)
    }
}
...
pub fn build_relay_router(state: RelayProxyState) -> Router {
    Router::new()
        .route(RELAY_HEALTH_PATH, get(relay_health))
        .route(RELAY_STOP_PATH, post(relay_stop))
        .route("/codex/{*path}", any(proxy_codex))
        .with_state(state)
}

async fn relay_health(State(state): State<RelayProxyState>) -> Response {
    match state.admin_control.as_ref() {
        Some(admin_control) => Json(admin_control.health_response()).into_response(),
        None => StatusCode::NOT_FOUND.into_response(),
    }
}

async fn relay_stop(State(state): State<RelayProxyState>, request: Request<Body>) -> Response {
    let Some(admin_control) = state.admin_control.as_ref() else {
        return StatusCode::NOT_FOUND.into_response();
    };
    let token = request
        .headers()
        .get(RELAY_ADMIN_TOKEN_HEADER)
        .and_then(|value| value.to_str().ok());
    if token != Some(admin_control.admin_token.as_str()) {
        return StatusCode::UNAUTHORIZED.into_response();
    }
    match admin_control.stop() {
